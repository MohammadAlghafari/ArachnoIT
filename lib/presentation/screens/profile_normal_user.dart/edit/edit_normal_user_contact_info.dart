...
class EditNormalUserContactInfo extends StatefulWidget {
  final NormalUserProfileDto info;
  final ProfileProviderBloc bloc;
  final ProfileInfoResponse profileInfoResponse;
  EditNormalUserContactInfo(
      {@required this.info,
      @required this.bloc,
      @required this.profileInfoResponse});
  @override
  State<StatefulWidget> createState() {
    return _EditNormalUserContactInfo();
  }
}

class _EditNormalUserContactInfo extends State<EditNormalUserContactInfo> {
          ...
          listener: (context, state) {
            if (state is GetAllCountriesState) {
              ...
              GlobalPurposeFunctions.showOrHideProgressDialog(context, false)
                  .then(
                (value) {
                  widget.bloc.add(UpdateNormalUserContactInfoManual(
  ...
  void _handleGetAllCountriesState(GetAllCountriesState state) {
    ...
    for (int i = 0; i < state.countriesList.length; i++) {
      seachItems.add(SearchModel(
        name: state.countriesList[i].name,
        id: state.countriesList[i].id,
      ));
    }
    showDialog(
        context: context,
            ...
            child: Container(
              ...
              child: Padding(
                ...
                child: Text(
                  ...
                  style: mediumMontserrat(color: Colors.white, fontSize: 16),
                ),
              ),
            ),
          ),
