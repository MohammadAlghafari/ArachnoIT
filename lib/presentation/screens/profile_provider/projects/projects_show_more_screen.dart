...
import 'package:arachnoit/presentation/screens/profile_provider/projects/update_project.dart';
...
class ProjectsShowMore extends StatefulWidget {
  final bool canMakeUpdate;
  final String userId;
  ProjectsShowMore({@required this.canMakeUpdate, @required this.userId});
  @override
  State<StatefulWidget> createState() {
    return _ProjectsShowMore();
  }
}

class _ProjectsShowMore extends State<ProjectsShowMore> {
  ...
  Widget showInfo(ProfileProviderProjectsState state) {
      ...
      body: ListView.builder(
        ...
        itemBuilder: (BuildContext context, int index) {
          ...
          return ProfileQualificationShowMoreListItem(
            ...
            titleItemName: AppLocalizations.of(context).projects_item_name,
            ...
            updateFunction: () {
              Navigator.of(context).push(MaterialPageRoute(
                  builder: (context) => UpdateProjectScreen(
                        prjoectItem: state.posts[index],
                        index: index,
                        bloc: profileProviderProjectsBloc,
                      )));
            },
            deleteFunction: () {
              GlobalPurposeFunctions.showOrHideProgressDialog(context, true);
              profileProviderProjectsBloc.add(
                  DeleteProjectItem(index: index, itemId: state.posts[index].id, context: context));
            },
          );
        },
      ),
  ...
  void _onScroll() {
    if (_isBottom && !isReloadData) {
      profileProviderProjectsBloc.add(GetAllProjects(newRequest: false, userId: widget.userId));
    }
  }
