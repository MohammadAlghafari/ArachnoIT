...
class UpdateProjectScreen extends StatefulWidget {
  final ProjectsResponse prjoectItem;
  final int index;
  final ProfileProviderProjectsBloc bloc;
  UpdateProjectScreen({
    @required this.prjoectItem,
    @required this.index,
    @required this.bloc,
  });
...
class _AddProjectScreen extends State<UpdateProjectScreen> {
  ...
  void initState() {
    ...
    startTime.text = DateFormat('yyyy-MM-dd').format(widget.prjoectItem.startDate);
    endTime.text = widget.prjoectItem.endDate.toString();
    _nameController.text = widget.prjoectItem.name;
    _urlController.text = widget.prjoectItem.url;
    _detailController.text = widget.prjoectItem.description;
    for (AttachmentResponse item in widget.prjoectItem.attachments) {
    ...
    if (endTime.text == "null") {
      ...
      endTime.text = DateFormat('yyyy-MM-dd').format(widget.prjoectItem.endDate);
    }
    startAndEndTimeWithExpire = StartAndEndTimeWithExpire(
      startTime: startTime,
      endTime: endTime,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBarProject.showAppBar(title: AppLocalizations.of(context).update_projects),
      body: BlocProvider<ProfileProviderProjectsBloc>(
        create: (context) => profileProviderProjectsBloc,
        child: BlocListener<ProfileProviderProjectsBloc, ProfileProviderProjectsState>(
          listener: (contex, state) {
            if (state is AddItemToFileListState) {
              file.addAll(state.file);
            } else if (state is RemoveItemFromFileListState) {
              removedItem.add(file[state.index].imageID);
              file.removeAt(state.index);
            } else if (state is SuccessUpdateProjects) {
              GlobalPurposeFunctions.showOrHideProgressDialog(context, false).then((value) {
                widget.bloc
                    .add(UpdateListAfterSuccess(index: widget.index, newItem: state.newProject));
                      ...
                      InkWell(
                        onTap: () async {
                          ...
                          if (result != null) {
                            ...
                            profileProviderProjectsBloc.add(AddItemToFileList(file: files));
                          } else {}
                        },
                        child: Container(
                          height: 60,
            ...
            onPressed: () {
              ...
              profileProviderProjectsBloc.add(
                UpdateSelectedProject(
                  ...
                  removedfiles: removedItem,
                  id: widget.prjoectItem.id,
                ),
              );
            }),
      ],
