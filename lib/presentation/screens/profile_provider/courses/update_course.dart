...
class UpdateCourse extends StatefulWidget {
  ...
  final ProfileProviderCoursesBloc bloc;
  final int index;
  UpdateCourse({@required this.course, @required this.bloc, @required this.index});
  @override
  _UpdateCourse createState() => new _UpdateCourse();
}

class _UpdateCourse extends State<UpdateCourse> {
  List<ImageType> file = [];
  ...
  void initState() {
    ...
    _placeController.text = widget.course.place;
    _dateOfBirthController.text = DateFormat('yyyy-MM-dd').format(widget.course.date);
    for (AttachmentResponse item in widget.course.attachments) {
      file.add(ImageType(
        isFromDataBase: true,
        imageID: item.id,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBarProject.showAppBar(title: AppLocalizations.of(context).update_courses),
        ...
        child: BlocListener<ProfileProviderCoursesBloc, ProfileProviderCoursesState>(
          listener: (contex, state) {
            ...
            if (state is AddItemToFileListState) {
              ...
              deleteItem.add(file[state.index].imageID);
              file.removeAt(state.index);
            } else if (state is SuccessUpdateCourses) {
              GlobalPurposeFunctions.showOrHideProgressDialog(context, false).then((value) {
                widget.bloc.add(UpdateDataAfterSuccess(
                    newCourseResponse: state.newCourseResponse, index: widget.index));
      ...
      children: [
        ...
        RaisedButton(
          ...
          onPressed: () {
            ...
            profileProviderCoursesBloc.add(UpdateSelectedCourse(
              file: file,
              context: context,
